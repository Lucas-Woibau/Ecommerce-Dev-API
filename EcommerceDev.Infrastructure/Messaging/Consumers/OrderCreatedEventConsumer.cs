        ...
        consumer.ReceivedAsync += async (model, eventArgs) =>
        {
            try
            {
                ...
                _logger.LogInformation(
                    "[Consumer] Received OrderCreatedEvent with Id {OrderId}",
                    @event.IdOrder);
                ...
                {
                    _logger.LogWarning(
                        "[Consumer] Order with Id {OrderId} does not exist",
                        @event.IdOrder);
                ...
                // Send payment link via SignalR to clients in this order's group
